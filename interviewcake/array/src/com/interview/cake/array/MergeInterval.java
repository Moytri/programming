 ...
 * 
 * @author Moytri
 * Question: https://www.interviewcake.com/question/java/merging-ranges?course=fc1&section=array-and-string-manipulation
...
public class MergeInterval {
    ...
    public static List<Meeting> mergeRanges(List<Meeting> meetings) {
        ...
        List<Meeting> mergeedMeetings = new ArrayList<>();
        ...
        for (int i = 0; i <= size - 1; i++) {
            ...
            for (int j = i + 1; j < size; j++) {
                if (!visited[j] && currentMeeting != null &&
                    !(currentMeeting.startTime > meetings.get(j).endTime || 
                    currentMeeting.endTime < meetings.get(j).startTime)) {
                    
                    currentMeeting.startTime = Math.min(currentMeeting.startTime, meetings.get(j).startTime);
                    currentMeeting.endTime = Math.max(currentMeeting.endTime, meetings.get(j).endTime);
                    visited[j] = true;
                }
            }
            
            if (currentMeeting != null) {
                mergeedMeetings.add(currentMeeting);
                currentMeeting = null;
            }
        }

        Collections.sort(mergeedMeetings, (a,b) -> a.startTime - b.startTime);
        return mergeedMeetings;
    }
    ...
    public static void main(String[] args) {
        Result result = JUnitCore.runClasses(MergeInterval.class);
